...
# set-firebase-domain.sh, set-sql-tier.sh, upgrade/downgrade-sql-replica.sh, and
...
#
# alert_notification_email deliberately does NOT live here — this repo is public, and unlike
# GCP_PROJECT_ID/GCP_REGION (also not secret, but harmless to expose), a personal email address
# genuinely shouldn't sit in permanent public commit history. It's passed to CI as the
# ALERT_NOTIFICATION_EMAIL repo variable instead (see ci.yml and infra/README.md's CI/CD
# section) — set once via `gh variable set`, never committed.
