...
// true to re-enable self-serve upgrade/downgrade/renew once live checkout is back, same idiom
// as CandidateBillingPage's ENABLE_PLAN_UPGRADE.
const ENABLE_PLAN_UPGRADE = false

const WHATSAPP_NUMBER = '918088076264'
const WHATSAPP_LINK = `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(
  'Hi, I want to activate a paid plan on OpenOpportunity.',
)}`
          ...
          return (
            <div
              ...
              className={`relative flex flex-col rounded-2xl border-2 bg-surface p-[26px] ${
                isCurrent ? 'border-teal' : 'border-border'
              }`}
            >
              {isCurrent && (
                <span className="absolute -top-[11px] left-6 rounded-full bg-teal px-3 py-[3px] text-[11px] font-bold text-white">
              ...
              <button
                type="button"
                disabled={
                  !ENABLE_PLAN_UPGRADE || (isCurrent && key === 'free') || changingPlan !== null
                }
